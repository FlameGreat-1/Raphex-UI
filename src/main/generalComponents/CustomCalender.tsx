...
const CustomCalender = (props: CalenderProps) => {
    ...
    return (
        <div className={`${closeCalender && 'hidden'} bg-transparent font-poppins w-full h-fit flex justify-center z-[100px]`}>
                    ...
                    <div className="col-span-7 grid grid-cols-7 gap-3 mt-4">
                        {daysInMonth && new Array(daysInMonth).fill(0).map((_, index) => (
                           (index === 0) && <p key={index} className={`${ (selectedDay ===index+1) && ' border-cosmic-color-lightBlue border'}  xs:w-[32px] cursor-default hover:border hover:border-cosmic-color-lightBlue w-[60px] p-1 md:m-1 text-center rounded-md xs:text-[10px]`} style={{ gridColumn: firstDay }} onClick={() => setSelectedDay(index + 1)}>{index + 1}</p> ||
                            (index + 1 === selectedDay) && <p key={index} className="xs:w-[32px] cursor-default hover:border hover:border-cosmic-color-lightBlue border border-cosmic-color-lightBlue w-[48px] p-1 md:m-1 text-center rounded-md xs:text-[10px]" onClick={() => setSelectedDay(index + 1)}>{index + 1}</p> ||
